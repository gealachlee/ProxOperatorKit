...
from prox.sep_prox.dev import Prox1over3, Prox1over2
...
lqlist = [
    container.prox_l1(), #'b'
    container.prox_l1over2(), #'r'
    container.prox_l2over3(), #'g'
     # 'c'
    Prox1over3()
    # Prox1over2()
]
...
l2_psi_prox_list: list[ProximalOperator] = [
 ...
 #    GeneralProxL2Psi(n=container.n(),gLen=container.gLen(),subvec_prox= container.prox_l1over2()),
    GeneralProxL2Psi(n=container.n(),gLen=container.gLen(),subvec_prox=Prox1over3())
]
...
for index, record in enumerate(total_results.results):
    ...
    ax1.plot(record.metrics, color=plot_color[index], linestyle=linestyle,label =record.desc)
    ax1.set_yscale('log')
